...
## v1.7.0 — инкрементальный экспорт хостов + ручной полный экспорт

`core`-экспорт при 10–15 тыс. хостов даже после оптимизаций каждый прогон
перегонял все хосты. Теперь регулярный прогон экспортирует только изменённые
хосты, а полный снимок выполняется отдельным ручным DAG.

### Что нового

- **Инкрементальный `core`** (`zabbix_core_to_gitlab_<env>`, `0 21 * * *`):
  экспортирует только хосты, изменённые за скользящее окно
  (`<env>_host_incremental_window_hours`, дефолт 24 ч), по данным
  `auditlog.get` (resourcetype host). Группы и макросы экспортируются всегда
  (они малы).
- **Полный ручной DAG** `zabbix_core_full_to_gitlab_<env>` (`schedule=None`):
  полный снимок всех хостов + **чистка осиротевших** файлов в `core/hosts/`
  (удаляет то, чего больше нет в Zabbix, в т.ч. старые имена без hostid от
  v1.6.4). Запускать вручную периодически и после массовых изменений.
- Различение «нет изменений» и «потеря сигнала»: если `auditlog.get`
  недоступен (нет прав/таймаут), инкремент НЕ считает молча, что менять
  нечего — логирует предупреждение и пропускает хосты (полный снимок даёт
  ручной DAG).

### Изменения в коде

- **`zabbix_client.py`** — `get_changed_hostids(window_sec, ...)` →
  `(set[hostid], audit_ok)`; константа `AUDIT_RESOURCE_HOST=4`
  (параметризуема).
- **`gitlab_client.py`** — `list_files(path)` (рекурсивный листинг с
  пагинацией) для вычисления осиротевших.
- **`config_sync.py`** — режим `full`|`incremental` у `ConfigSynchronizer`;
  фильтрация хостов по auditlog в инкременте; `_cleanup_orphans` (delete
  лишних) только при full для core; `ConfigSyncStats.deleted`.
- **`dag_factory.py`** — параметры `mode`/`schedule=None`/`dag_id_template`;
  Variable `<env>_host_incremental_window_hours`.
- **`dags/zabbix_core_to_gitlab.py`** → incremental;
  **`dags/zabbix_core_full_to_gitlab.py`** (новый) → full+cleanup, ручной.

### Эксплуатация

- Один раз запустите **full**-DAG, чтобы снять полный снимок и подчистить
  осиротевшие файлы (включая старые имена от переименования v1.6.4). Дальше
  ежедневный инкремент держит репозиторий в актуальном состоянии, а full —
  периодически (например, раз в неделю) и после крупных изменений.

### Совместимость

- **Zabbix 7.4.5 / Airflow 3.1.2** — auditlog.get/tree в форме API 7.4/v4;
  новый ручной DAG по паттернам Airflow 3 (`schedule=None`). 21 smoke-тест.
  Проверьте, что resourcetype host в вашей версии = 4 (значение по
  умолчанию); при отличии задаётся в коде.

## v1.6.4 — уникальные имена файлов хостов (фикс коллизии коммита)

После фикса 413 (v1.6.3) `core`-экспорт всё ещё падал на одной пачке (150
ошибок). Причина — НЕ размер, а коллизия имён файлов: два разных хоста Zabbix
(`yac-vw-00131_MI1609M` и `yac-vw-00131_MI1609M-`) после `safe_filename`
схлопывались в один путь `core/hosts/yac-vw-00131_MI1609M.yaml`. GitLab
отклоняет коммит с двумя действиями на один файл, поэтому падала вся пачка с
дублем (ретраи бесполезны — ошибка детерминированная).

### Что изменилось

- **Имя файла хоста → `<имя>_<hostid>.yaml`** (раньше `<имя>.yaml`). hostid
  гарантирует уникальность независимо от того, как имена хостов
  санитизируются. `export_hosts_batched`/`_slice_hosts_export` теперь
  отдают `(host_name, hostid, yaml)`; hostid сопоставляется по
  техническому имени из `host.get` (configuration.export сам hostid не
  включает).
- **Дедупликация действий по `file_path`** в `commit_multiple` — страховка:
  если в один коммит попадут два действия на один путь, остаётся последнее
  (а не падает вся пачка).
- **Реальная причина ошибки коммита** вместо «commit failed»: пачки,
  упавшие после ретраев, возвращаются как `(file_path, reason)`, где
  reason содержит HTTP-код/фрагмент тела ответа. Контракт `commit_multiple`
  изменён: `list[tuple[file_path, reason]]`.

### ⚠️ Разовое переименование при первом прогоне

Смена схемы имён означает, что при первом прогоне после обновления все
файлы хостов будут пересозданы под новыми именами (`<имя>_<hostid>.yaml`),
а старые (`<имя>.yaml`) останутся как осиротевшие. Это ожидаемый разовый
шумный коммит. Очистку осиротевших файлов выполнит будущий full-экспорт
(этап инкрементальности) либо разовая ручная чистка каталога `core/hosts/`.

### Совместимость

- **Zabbix 7.4.5 / Airflow 3.1.2** — затронут core-экспорт хостов и слой
  коммита; 19 smoke-тестов проходят.

## v1.6.3 — чанкование коммитов (фикс 413 Request too large)

При экспорте 10–15 тыс. хостов весь набор файлов уходил одним POST
`/repository/commits`. На таком объёме запрос отбивался WAF/прокси (Qrator)
с `413 Request is too large` — таск падал с тысячами ошибок, в git не
попадало ничего.

### Что изменилось

- **`commit_multiple` дробит действия на пачки** по `commit_chunk_size`
  (дефолт 150) и шлёт отдельным коммитом каждую (`... (part k/n)` в
  сообщении). Тело каждого запроса укладывается в лимит прокси.
- **Ретрай пачки** до `commit_max_retries` раз (дефолт 3) с паузой; если
  пачка не прошла — её файлы помечаются неудавшимися, обработка
  **продолжается** со следующими (для DR важно записать максимум данных).
- **Изменён контракт `commit_multiple`**: вместо исключения на ошибке
  возвращает список путей файлов, которые не удалось закоммитить.
  Синхронизаторы (templates/auth/config) переносят эти файлы в `stats.errors`
  и убирают из created/updated.
- Параметры настраиваются: `SyncConfig.commit_chunk_size`/
  `commit_max_retries`, Variable `<env>_commit_chunk_size`/
  `<env>_commit_max_retries`, env `COMMIT_CHUNK_SIZE`/`COMMIT_MAX_RETRIES`,
  yaml `commit_chunk_size`/`commit_max_retries`.

### Совместимость

- **Zabbix 7.4.5 / Airflow 3.1.2** — затронут слой записи в GitLab, не код
  экспорта. Применяется ко всем синхронизаторам (templates, auth, config).
  18 smoke-тестов проходят.
- Семантика `single_commit` теперь «изменения уходят чанк-коммитами», а не
  строго одним коммитом (атомарность по построению невозможна при тысячах
  файлов).
