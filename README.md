...
Хосты выгружаются по файлу на каждый (`core/hosts/<имя>_<hostid>.yaml`) через
`configuration.export`. hostid в имени гарантирует уникальность: разные хосты
с близкими именами (например, `X` и `X-`) после очистки спецсимволов дали бы
один путь и сломали коммит. Для производительности экспорт идёт **пачками**
...
по аналогии с таймаутом auditlog.

Коммиты в GitLab дробятся на пачки по `<env>_commit_chunk_size` файлов
(дефолт 150) с ретраем `<env>_commit_max_retries` (дефолт 3): один POST со
всеми тысячами файлов отбивается WAF/прокси (Qrator) с `413 Request too
large`, поэтому изменения уходят чанк-коммитами. Сбойная пачка повторяется,
затем — если не прошла — её файлы попадают в ошибки прогона, а остальные
коммитятся. Группы и media types
...
**Хосты — инкрементально.** Регулярный `core`-DAG экспортирует только хосты,
изменённые за скользящее окно (`<env>_host_incremental_window_hours`, дефолт
24 ч) по данным `auditlog.get`. Полный снимок всех хостов и чистку осиротевших
файлов (которых больше нет в Zabbix) делает отдельный **ручной** DAG
`zabbix_core_full_to_gitlab_<env>` (`schedule=None`) — запускайте его
периодически (например, раз в неделю) и после массовых изменений. Если
`auditlog.get` недоступен, инкремент пропускает хосты с предупреждением и
полагается на ручной полный экспорт, а не молчаливо считает, что изменений нет.
